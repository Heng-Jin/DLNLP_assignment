...
model_save_path = parent_path / ("BERT_5class_train_" + "epoch" + str(num_epochs) + "_lr" + str(learning_rate) + str(time.strftime("_%m_%d_%H_%M", time.localtime())))
model_save_path.mkdir(exist_ok=True)  # all outputs of this running will be saved in this path
log_path = model_save_path / ("BERT_5class_train_" + str(time.strftime("%m_%d_%H_%M_%S", time.localtime())) + ".log")
