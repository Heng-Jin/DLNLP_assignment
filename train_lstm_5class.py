...
from datasets import CSVJSONDataset
...
train_data_path = pathlib.Path.cwd()/ "Kaggle_news_train_5class.csv"
val_data_path = pathlib.Path.cwd()/ "Kaggle_news_test_5class.csv"
train_data_folder = pathlib.Path.cwd() / "Kaggle_news_train"
val_data_folder = pathlib.Path.cwd() / "Kaggle_news_test"

# computing set up
os.environ["CUDA_VISIBLE_DEVICES"] = "2" # GPU assigned
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# logging set up
parent_path = pathlib.Path.cwd()
model_save_path = parent_path / ("LSTM_train_5class_" + "epoch" + str(num_epochs) + "_lr" + str(learning_rate)  + str(time.strftime("_%m_%d_%H_%M", time.localtime())))
...
# 数据预处理，加载数据集
# 请根据实际任务加载并预处理数据
train_dataset = CSVJSONDataset(train_data_path, train_data_folder)
val_dataset = CSVJSONDataset(val_data_path, val_data_folder)
